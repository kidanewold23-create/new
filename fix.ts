...
const refHandlerStart = newText.indexOf('if (isMenuCommand(text, "menu_refer_friend")');
if (refHandlerStart > -1) {
  const refHandlerEnd = newText.indexOf('return new Response("OK"', refHandlerStart);
  const nextBracket = newText.indexOf('}', refHandlerEnd) + 1;
  ...
  newText = newText.slice(0, nextBracket) + toInsert + newText.slice(nextBracket);
}
...
const startGen = newText.indexOf('import fontkit');
const endGen = newText.indexOf('async function checkAndApplyReferralReward');

const newFunc = `import * as fontkitLib from 'https://esm.sh/@pdf-lib/fontkit@1.1.1';
