...
if (startGen > -1 && endGen > -1) {
  ...
  console.log("Could not find startGen or endGen. startGen:", startGen, "endGen:", endGen);
}

newText = newText.replaceAll('kickUserFromChannel', 'removeUserFromChannel');

// Explicitly remove all internal occurrences of 'const { data: reg } = ...' inside the handler functions manually if replaceAll didn't match perfectly due to whitespace:
newText = newText.replaceAll('      const { data: reg } = await supabase.from("registrations").select("*").eq("chat_id", chatId).maybeSingle();\n      const [lang] = getLangAndStep(reg);', '');
newText = newText.replaceAll('      const { data: reg } = await supabase.from("registrations").select("*").eq("chat_id", chatId).maybeSingle();\r\n      const [lang] = getLangAndStep(reg);', '');
