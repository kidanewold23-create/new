...
if (typeof globalThis !== "undefined") {
  (globalThis as any).generateOneTimeTelegramInviteLink = generateOneTimeTelegramInviteLink;
}
...
export default async function handler(req: any, res?: any) {
  ...
  const sendRes = (bodyData: any, statusCode: number = 200, extraHeaders: Record<string, string> = {}) => {
  ...
  const getJsonBody = async (): Promise<any> => {
...
`;
    ...
    if (pathname === "/api/admin/broadcast" && reqMethod === "POST") {
      try {
        ...
        const students = await dbStore.getStudents();
        let telegramRecipients = students.filter((s: any) => s.id && (String(s.id).startsWith("TG-") || /^\d+$/.test(String(s.id))));
    ...
    if (pathname === "/api/analytics" && reqMethod === "GET") {
      ...
      const completedTxns = transactions.filter((t: any) => 
      ...
      const dailyRevenue: Record<string, number[]> = { all: [] };
      const dailyEnrollments: Record<string, number[]> = { all: [] };
      const monthlyRevenue: Record<string, number[]> = { all: [] };
      const monthlyEnrollments: Record<string, number[]> = { all: [] };
      const yearlyRevenue: Record<string, number[]> = { all: [] };
      const yearlyEnrollments: Record<string, number[]> = { all: [] };
      ...
      dailyRevenue.all = dailyLabels.map((_, i) => courses.reduce((acc: number, c: any) => acc + (dailyRevenue[c.id]?.[i] || 0), 0) || Math.round(dailyWeights[i] * grossRevenue * 0.12));
      dailyEnrollments.all = dailyLabels.map((_, i) => courses.reduce((acc: number, c: any) => acc + (dailyEnrollments[c.id]?.[i] || 0), 0) || Math.max(1, Math.round(dailyWeights[i] * totalStudentsCount * 0.12)));
      monthlyRevenue.all = monthlyLabels.map((_, i) => courses.reduce((acc: number, c: any) => acc + (monthlyRevenue[c.id]?.[i] || 0), 0) || Math.round(monthlyWeights[i] * grossRevenue));
      monthlyEnrollments.all = monthlyLabels.map((_, i) => courses.reduce((acc: number, c: any) => acc + (monthlyEnrollments[c.id]?.[i] || 0), 0) || Math.max(1, Math.round(monthlyWeights[i] * totalStudentsCount)));
      yearlyRevenue.all = yearlyLabels.map((_, i) => courses.reduce((acc: number, c: any) => acc + (yearlyRevenue[c.id]?.[i] || 0), 0) || Math.round(yearlyWeights[i] * grossRevenue));
      yearlyEnrollments.all = yearlyLabels.map((_, i) => courses.reduce((acc: number, c: any) => acc + (yearlyEnrollments[c.id]?.[i] || 0), 0) || Math.max(1, Math.round(yearlyWeights[i] * totalStudentsCount)));

      const recentEnrollments = transactions.slice(0, 15).map((t: any) => ({
        id: t.id,
      ...
      return sendRes({
        ...
        data: {
          ...
          courses: courses.map((c: any) => ({ id: c.id, title: c.title, category: c.category, price: c.price, enrolled_students: c.enrolled_students || 0, color: courseStats[c.id]?.color || "#f59e0b" })),
          trajectories: {
    ...
    if (pathname === "/api/verify/transaction" && reqMethod === "POST") {
      try {
        ...
        const courses = await dbStore.getCourses();
        const course = courses.find((c: any) => c.id === courseId || c.title === courseId) || courses[0];
        ...
        const alreadyUsed = existingTxns.find((t: any) => 
          (t.status === "Completed" || t.status === "VERIFIED" || t.status === "Settled") &&
        ...
        try {
          ...
          if (!linkGenFn && typeof (globalThis as any).generateOneTimeTelegramInviteLink === "function") {
            linkGenFn = (globalThis as any).generateOneTimeTelegramInviteLink;
          }
        ...
        if (BOT_TOKEN && studentPhone) {
          try {
            ...
            if (cleanLast9) {
              const students = await dbStore.getStudents();
              const matchingStu = students.find((s: any) => {
    ...
    if (pathname.startsWith("/api/verify/status/") && reqMethod === "GET") {
      ...
      const match = txns.find((t: any) => String(t.reference_number || t.id).toUpperCase() === refId.toUpperCase());
