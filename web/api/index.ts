...
`;
  ...
  if (pathname === "/api/student/telegram-auth" && req.method === "POST") {
    try {
      const body = await req.json();
      const fn = (dbStore as any).telegramAuthLogin || (dbStore as any).authenticateTelegramUser;
  ...
  if (pathname === "/api/verify/transaction" && req.method === "POST") {
    try {
      ...
      try {
        let linkGenFn: any = typeof generateOneTimeTelegramInviteLink === "function" ? generateOneTimeTelegramInviteLink : null;
