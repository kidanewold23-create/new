...
newText = newText.replace(
  ...
  'const { data: regB } = await supabase.from("registrations").select("*").eq("chat_id", chatId).order("created_at", { ascending: false }).limit(1).maybeSingle();\n      const msDiff = regB ? (Date.now() - new Date(regB.created_at).getTime()) : 0;\n      const daysSinceReg = regB ? (isTest1Min ? Math.floor(msDiff / (60 * 1000)) + 1 : Math.floor(msDiff / (24 * 3600 * 1000)) + 1) : 1;'
);
newText = newText.replace(
