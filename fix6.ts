...
for (let i = 0; i < lines.length; i++) {
  const line = lines[i];
  if (line.includes('const { data: reg } = await supabase.from("registrations").select("*").eq("chat_id", chatId).order("created_at", { ascending: false }).limit(1).maybeSingle();') || 
      line.includes('const { data: reg } = await supabase.from("registrations").select("*").eq("chat_id", chatId).maybeSingle();')) {
    // Only remove inside sendNextQuizQuestion block
    // actually, those are duplicates anyway because the parent function already defined them!
    // But wait, the function `sendNextQuizQuestion` doesn't define them at the top!
    // Let me check if sendNextQuizQuestion defined them at the top.
    // In 2acf2b4: No, `sendNextQuizQuestion` defined them EXACTLY twice!
  }
}

// Safer approach: 
// In sendNextQuizQuestion, `reg` and `lang` were NOT block scoped, they were function scoped, but Deno type checker thinks they are block-scoped redeclared.
// Let's just rename the duplicates inside `sendNextQuizQuestion` to `reg2` and `lang2`!
newText = newText.replace(
...
// Wait, the variables are used afterwards! 
// Let's just replace `const { data: reg }` with `{ data: reg }` and `const [lang]` with `[lang]`? No, typescript won't allow reassignment of const!
// I'll change the FIRST declaration to `let reg: any; let lang: string;` and reassign it.
// Actually, it's easier to just replace `const { data: reg }` with `let { data: reg }` across the function, and later reassign.
// Wait, the easiest way to fix "Cannot redeclare block-scoped variable" is to just REMOVE the `const ` from the subsequent ones! But they are constants.
// Let's just wrap the inner blocks in `{ }` or rename them!
