...
export const dbStore = {
  ...
  registerStudentAccount: async ({ name, phone, email, password }) => {
    ...
    const existing = allStudents.find(s => {
      ...
      const cDigits = cleanPhone.replace(/[^0-9]/g, "");
      if (sDigits && cDigits && sDigits.includes(cDigits)) return true;
      if (cleanEmail && s.email && s.email.toLowerCase() === cleanEmail) return true;
      return false;
    });

    if (existing && existing.password_hash) {
      return { success: false, error: "An account with this phone number or email already exists. Please log in." };
    }
    ...
    const studentData = {
      ...
      name: cleanName,
      phone: cleanPhone,
      email: cleanEmail || `${cleanPhone.replace(/[^0-9]/g, "")}@founders.et`,
  ...
  authenticateStudent: async ({ identifier, password }) => {
    ...
    const match = allStudents.find(s => {
      ...
      if (cleanIdDigits && cleanIdDigits.length >= 8 && sPhoneDigits.includes(cleanIdDigits)) return true;
      if (sEmail && sEmail === cleanId) return true;
      if (sId && sId === cleanId) return true;
      return false;
    });

    if (!match) {
      return { success: false, error: "No student account found with this phone/email. Please sign up first." };
    }
  ...
  authenticateTelegramUser: async (tgData) => {
    if (!tgData || (!tgData.id && !tgData.telegram_id)) {
      return { success: false, error: "Invalid Telegram authentication payload." };
    }

    // Secure HMAC SHA-256 validation if hash is provided
    const BOT_TOKEN = (typeof process !== "undefined" && process.env ? (process.env.TELEGRAM_BOT_TOKEN || process.env.BOT_TOKEN) : "") || (typeof Deno !== "undefined" && Deno.env ? Deno.env.get("TELEGRAM_BOT_TOKEN") || Deno.env.get("BOT_TOKEN") : "");
    if (tgData.hash && BOT_TOKEN) {
      try {
        const cryptoModule = await import("node:crypto").catch(() => null);
        if (cryptoModule && cryptoModule.createHash) {
          ...
          const secretKey = cryptoModule.createHash("sha256").update(BOT_TOKEN).digest();
          const calculatedHash = cryptoModule.createHmac("sha256", secretKey).update(dataCheckString).digest("hex");
          if (calculatedHash !== tgData.hash) {
            return { success: false, error: "Security validation failed: Invalid Telegram authentication signature." };
          }
        }
      } catch (_secErr) {}
    }

    const tgIdStr = String(tgData.id || tgData.telegram_id).trim();
...
const defaultLandingConfig = {
  ...
  whatYouGet: {
    ...
    deliverables: [
      ...
      },
      {
        ...
        desc: "Access the proprietary @FoundersAcademyBot for instant module delivery, homework quizzes, and one-click support links.",
  ...
  faqs: [
    ...
    },
    {
      ...
      answer: "Payment is instantly verified via Verify.ET (Telebirr & CBE). Once verified, you immediately receive automated one-time access links to our private Telegram classroom channels and @FoundersAcademyBot.",
      status: "active"
    },
    {
      id: "faq-3",
